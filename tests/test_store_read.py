...
def test_connect_readonly_refuses_an_older_database_in_one_line(tmp_path):
    ...
    assert str(err.value) == "store is at schema version 1, older than the 6 this code expects"
...
def test_connect_readonly_refuses_a_newer_database(tmp_path):
    ...
    put(c, "schema_version", version=7, applied_at=NOW, description="future")
    c.close()
    with pytest.raises(read.StoreVersionError) as err:
        read.connect_readonly(url)
    assert str(err.value) == "store is at schema version 7, newer than the 6 this code expects"
